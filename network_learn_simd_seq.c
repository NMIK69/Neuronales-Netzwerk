...
void fp_matrix_mult_simd_seq(struct Network* nw, struct Matrix* m, struct Layer* l_prev, struct Layer* l) {
    ...
    for(int i = 0; i < m->H; i++) {
        temp = dot_product(l_prev->a, m->matrix[i], m->W);
...
void forward_prop_simd_seq(struct Network* nw) {
    ...
    for(int i = 0; i < nw->layer_num - 1; i++) {
        fp_matrix_mult_simd_seq(nw, nw->weights[i], nw->layers[i], nw->layers[i + 1]);
    }
...
void backwards_prop_simd_seq(struct Network* nw) {
    ...
    backwards_prop_output_layer_simd_seq(nw);

    int hidden_layer_index = nw->layer_num - 2;

    for(int i = hidden_layer_index; i > 0; i--) {
        backwards_prop_hidden_layer_simd_seq(nw, i);
    }
